    ...
    for (let key of formData.keys()) {
      ...
      if (key == "assigned_athlete") {
        ...
        if (mealData["owner_username"] != sessionStorage.getItem("username")) {
          ...
          athlete_option.innerText = "You";
          document.getElementById("inputAthlete").appendChild(athlete_option);
        }
        document.getElementById("inputAthlete").value = mealData[key];
        document.getElementById("inputAthlete").disabled = true;
        continue;
      }
...
function generateMealForm() {
  ...
  let athlete = null;
  if (formData.get("assigned_athlete") != "Select athlete") {
    athlete = formData.get("assigned_athlete");
  }
  submitForm.append("assigned_athlete", athlete);
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  for (let athleteUrl of currentUser.athletes) {
    ...
    let athlete_option = document.createElement("option");
    athlete_option.value = athlete.url;
    athlete_option.innerText = athlete.username;
    console.log(athlete);
    document.getElementById("inputAthlete").appendChild(athlete_option);
  }
  ...
  if (urlParams.has("id")) {
    ...
    cancelMealButton.className = cancelMealButton.className.replace(
      " hide",
      ""
    );
